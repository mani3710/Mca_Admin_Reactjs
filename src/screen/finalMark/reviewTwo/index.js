...
import './index.css';
import { setSelectedBatchForMoreDetails, getReviewList, getFirstReviewMarks, getReviewTwoMarks } from '../../../redux/reducer/projectlist';
...
const ReviewTwo = (props) => {
    ...
    useEffect(() => {
        dispatch(getReviewTwoMarks(selectedBatchForMoreDetailsObj.uuid));
    }, [])



    const getTotalMark = (item) => {
        return parseFloat(item.detailesdesignmark) + parseFloat(item.implementationmark) + parseFloat(item.performanceandresultmark) + parseFloat(item.presentationandreportmark)
    ...
    const exportMarkWithOutDetails = () => {
        ...
        const title = `${selectedBatchForMoreDetailsObj.title} review 2 marks`;
        const headers = [["ROLL", "Name", "Mark"]];

        const data = secondReviewMarkList.map(elt => [elt.rollno, elt.name, getTotalMark(elt)]);
        ...
        doc.save(`${selectedBatchForMoreDetailsObj.title} review 2.pdf`)
    }
    ...
    const exportMarkWithDetails = () => {
        ...
        const data = secondReviewMarkList.map(elt => [
            elt.name,
            elt.rollno,
            parseFloat(elt.detailesdesignmark).toFixed(1),
        ...
        doc.text(title, marginLeft, 40);
        doc.autoTable(content);
        doc.save(`${selectedBatchForMoreDetailsObj.title} Review 2 Details.pdf`)
    }
        ...
        <center>
            <div class="height-100 bg-light" style={{ width: "95%", backgroundColor: "red" }}>
                ...
                <table class="table">
                    ...
                    <tbody>
                        {secondReviewMarkList.map((item, i) => {
                            return (
                                <tr>
                                    <th scope="col">{i + 1}</th>
                                    <th scope="col">{item.rollno}</th>
                                    <th scope="col" >{item.name}</th>
                                    <th scope="col" >{parseFloat(item.detailesdesignmark).toFixed(1)}</th>
